...
export async function getBuyTx(solanaConnection: Connection, wallet: WalletContextState, baseMint: PublicKey, quoteMint: PublicKey, amount: number, targetPool: string, customSlippage: number) {
  ...
  const slippage = new Percent(Math.floor(customSlippage * 100), 10000)
...
export async function getSellTx(solanaConnection: Connection, wallet: WalletContextState, baseMint: PublicKey, quoteMint: PublicKey, amount: string, targetPool: string, customSlippage: number) {
  try {
    ...
    const quoteToken = new Token(TOKEN_PROGRAM_ID, quoteMint, 9)
    const baseTokenAmount = new TokenAmount(baseToken, Math.floor(parseFloat(amount) * 10 ** tokenBal.value.decimals))
    const slippage = new Percent(Math.floor(customSlippage * 100), 10000)
    const walletTokenAccounts = await getWalletTokenAccount(solanaConnection, wallet.publicKey)
    ...
    console.log("Error while getting sell transaction ", error)
  }
}
