...
const ImageUpload: React.FC<ImageUploadProps> = ({ header, setFilePreview, setFileUrl, type }) => {
  ...
  return (
    <div className="flex flex-col justify-between gap-6 w-full">
      <label className="flex flex-row gap-1 font-semibold text-white text-lg">{header}</label>
      <input
        type="file"
        accept={type}
        ref={fileInputRef}
      ...
      />
      <div className="flex flex-col justify-between items-start gap-2 p-3 border-[1px] border-white rounded-lg w-full">
        {(imagePreview !== "https://scarlet-extra-cat-880.mypinata.cloud/" && imagePreview !== "" && imagePreview !== undefined && imagePreview !== null && imagePreview) ? (
          ...
          <div onClick={() => fileInputRef.current?.click()} className="flex flex-col justify-center items-center p-3 border-[1px] border-white border-dashed rounded-lg w-full h-48 overflow-hidden text-white cursor-pointer">
            <GoUpload className="text-white/60 text-4xl" />
